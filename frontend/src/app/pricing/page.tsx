...
export default function PricingPage() {
    ...
    return (
        <div className="min-h-screen bg-[#0B0C15] text-white selection:bg-purple-500/30 overflow-x-hidden flex flex-col items-center">
            ...
            <div className="relative font-sans w-full max-w-[1400px] mx-auto px-4 py-20 md:py-32 z-10 flex flex-col items-center gap-24">
                ...
                <div className="grid md:grid-cols-2 gap-8 lg:gap-16 w-full max-w-7xl mx-auto items-start">
                    ...
                    <motion.div
                        ...
                        className="bg-[#12131e]/80 backdrop-blur-md rounded-[40px] border border-white/5 p-8 md:p-12 flex flex-col h-full hover:bg-[#151625] transition-colors duration-300"
                    >
                        <h3 className="text-3xl font-bold text-gray-300 mb-2">Iniciante</h3>
                        ...
                        <p className="text-gray-400 text-lg leading-relaxed mb-10 h-14">
                            Funcionalidades essenciais para quem está começando a organizar a carteira.
                        </p>

                        <button
                            disabled
                            className="w-full py-5 rounded-2xl font-bold bg-white/5 text-gray-500 border border-white/5 uppercase text-sm tracking-widest cursor-not-allowed mb-10"
                        >
                            Seu Plano Atual
                        </button>

                        <div className="space-y-5 flex-1">
                            <p className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-4">O que está incluso</p>
                            {features.map((feature, i) => (
                                <div key={i} className="flex items-start gap-4 group">
                                    {feature.free ? (
                                        <div className="p-1 rounded-full bg-white/5 shrink-0 mt-0.5">
                                            <Check className="w-4 h-4 text-gray-300" />
                                        </div>
                                    ) : (
                                        <div className="p-1 shrink-0 mt-0.5 opacity-30">
                                            <X className="w-4 h-4 text-gray-500" />
                                        </div>
                                    )}
                                    <span className={`${feature.free ? 'text-gray-300' : 'text-gray-600 decoration-gray-700/50'} text-base leading-snug`}>
                                        {feature.name}
                                    </span>
                                </div>
                            ))}
                    ...
                    >
                        ...
                        <div className="absolute -inset-[3px] bg-gradient-to-tr from-purple-600 via-pink-600 to-amber-400 rounded-[44px] blur-[20px] opacity-40 group-hover:opacity-80 transition duration-700 animate-pulse-slow"></div>

                        <div className="relative bg-[#161726] rounded-[40px] border border-white/10 p-8 md:p-12 overflow-hidden h-full shadow-2xl shadow-purple-900/20">

                            {/* Recommended Badge - Floating Absolute */}
                            <div className="absolute top-0 right-0 p-8">
                                <span className="inline-flex items-center gap-1.5 px-4 py-1.5 bg-gradient-to-r from-amber-400 to-orange-500 text-black text-xs font-black uppercase tracking-widest rounded-full shadow-lg shadow-amber-500/20">
                                    <Crown className="w-3.5 h-3.5 fill-black" /> Recomendado
                                </span>
                            </div>

                            <div className="mb-2 flex items-center gap-3">
                                <span className="p-2 rounded-lg bg-gradient-to-br from-purple-500/20 to-pink-500/20 border border-purple-500/30">
                                    <Zap className="w-6 h-6 text-amber-400 fill-amber-400" />
                                </span>
                                <h3 className="text-3xl font-bold text-white">Investidor Pro</h3>
                            </div>

                            <div className="flex items-baseline gap-2 mb-6 ml-1">
                                <span className="text-6xl font-black text-white tracking-tighter">R$ 29,90</span>
                                <span className="text-gray-400 text-xl font-medium">/mês</span>
                            </div>

                            <p className="text-purple-200/90 text-lg leading-relaxed mb-10 h-14 pr-8">
                                A suíte completa para quem busca liberdade financeira real. Acesso ilimitado.
                            </p>

                            <button
                                onClick={handleSubscribe}
                                disabled={isLoadingCheckout || user?.is_premium}
                                className={`w-full py-5 rounded-2xl font-black text-base uppercase tracking-widest transition-all transform hover:scale-[1.02] shadow-2xl mb-10 group/btn flex items-center justify-center gap-3 relative overflow-hidden ${user?.is_premium
                            ...
                            >
                                ...
                                {(!isLoadingCheckout && !user?.is_premium) && <Crown className="w-5 h-5 relative z-10 mb-1" />}
                            </button>

                            <div className="space-y-5">
                                <p className="text-xs font-bold text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-400 uppercase tracking-widest mb-4">Tudo do iniciante, mais:</p>
                                {features.map((feature, i) => (
                                    <div key={i} className="flex items-start gap-4">
                                        {feature.premium ? (
                                            <div className="p-1 rounded-full bg-green-500/10 shrink-0 mt-0.5 border border-green-500/20">
                                        ...
                                        <span className={`${feature.premium ? 'text-white font-medium' : 'text-gray-600'} text-base leading-snug`}>
                                            {feature.name}
                                        </span>
                                    </div>
                                ))}
                            ...
                            <div className="mt-8 pt-6 border-t border-white/5 flex items-center justify-center gap-2 text-xs text-green-400/80 font-mono">
